...
All 4 photos of KJ are real. When placing:
- Always apply dark filter: `filter: brightness(0.65-0.75) contrast(1.05-1.1)`
- Use `object-fit: cover` with `object-position: center top` for portrait shots
- Add gradient overlays for text legibility:
  ```css
  background: linear-gradient(to right, transparent 50%, rgba(8,8,8,0.9) 100%);
  ```
- On hover: `transform: scale(1.03)` with `transition: 0.5s`
- Never crop KJ's face out of frame on hero/featured placements
...
- [x] All 4 KJ photos embedded and placed
...
- [ ] Extract base64 images into /images folder
